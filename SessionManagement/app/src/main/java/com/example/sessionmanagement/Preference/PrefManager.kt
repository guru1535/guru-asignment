package com.example.sessionmanagement.Preference
...
class PrefManager(context: Context) {
   ...
   private var editor = sharedPreferences.edit()
    ...
    fun logout(){
        ...
        editor.clear()               // one click all data clear
        editor.commit()


      /*  editor.remove(KEY_FIRSTNAME)
        editor.remove(KEY_LASTNAME)             // perticuler one item remove in click
        editor.remove(KEY_EMAIL)
        editor.remove(KEY_CONTACT)
        editor.remove(KEY_PASSWORD)
        editor.remove(KEY_CONFIRMPASSWORD)*/
